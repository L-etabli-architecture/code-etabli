...
const TOP_K = 8
// Écart max entre le meilleur score et les autres sources retenues
const MAX_SCORE_GAP = 0.12
const SOURCE_INSUFFISANTE = 'Source insuffisante — validation humaine requise.'

const SYSTEM_PROMPT = `Tu es un assistant spécialisé en codes du bâtiment et règlements d'urbanisme québécois pour la firme L'Établi Architecture & Design à Saint-Hyacinthe, QC.
...
export async function performRAG(
  ...
  // 3b. Filtrer les sources trop éloignées du meilleur score
  const filteredResults = rawResults.filter(
        (r) => Number(r.similarity) >= maxSimilarity - MAX_SCORE_GAP
      )

  // 4. Construire le contexte pour Claude
  const sources: SearchSource[] = filteredResults.map((r) => ({
